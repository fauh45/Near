...
#define T0L 34 // low time for either bit
#define T1L 18
...
esp_err_t ws2812_write_leds(struct led_state new_state) {
  ...
  ESP_RETURN_ON_ERROR(rmt_write_items(LED_RMT_TX_CHANNEL, led_data_buffer,
                                      LED_BUFFER_ITEMS, false),
                      TAG, "Failed to write items");
